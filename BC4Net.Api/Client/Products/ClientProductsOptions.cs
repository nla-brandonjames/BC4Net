...
namespace BigCommerce4Net.Api.ResourceClients
{
    public class ClientProductsOptions : 
        ...
        IChildResourceCount
    {
        public ClientProductsOptions(Configuration configuration, BCAuthentication authentication) : base(configuration, authentication) { }
        ...
        public async Task<IClientResponse<List<ProductsOption>>> GetAsync(int productid)
        {
            string resourceEndpoint = string.Format("products/{0}/options", productid);
            return await GetDataAsync<List<ProductsOption>>(resourceEndpoint);
        }
        public async Task<IClientResponse<ProductsOption>> GetAsync(int productid, int fieldId)
        {
            string resourceEndpoint = string.Format("products/{0}/options/{1}", productid, fieldId);
            return await GetDataAsync<ProductsOption>(resourceEndpoint);
        }
        public async Task<IClientResponse<List<ProductsOption>>> GetAsync(string resourceEndPoint)
        {
            return await GetDataAsync<List<ProductsOption>>(resourceEndPoint);
        }
        public async Task<IClientResponse<List<ProductsOption>>> GetAsync(string resourceEndPoint, IFilter filter)
        {
            return await GetDataAsync<List<ProductsOption>>(resourceEndPoint, filter);
        }
